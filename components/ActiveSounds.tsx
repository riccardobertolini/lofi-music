...
import { ControlBar, TimerText, SoundsActiveText, StopAllButton, VolumeIcon, VolumeControl, Row } from './ActiveSounds.style';
...
const ActiveSounds: FC<ActiveSoundsProps> = ({ timer, minutes, activeSounds, stopAll, setMasterVolume }) => {
    ...
    return (
        <ControlBar style={{ opacity: opacityValue }}>
            <Row>
                <TimerText>
                    {minutes < 10 ? `0${minutes}` : minutes}:{timer < 10 ? `0${timer}` : timer}
                </TimerText>
                <SoundsActiveText>
